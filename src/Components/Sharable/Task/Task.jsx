...
export default function Task({task, updateTask, updateState, deleteTask}) {
    ...
    function editTask() {
        updateState({openTask: true, mode:'edit', selectedId: task.id});
    }

    return <div className={`task whiteBg flex justifyBetween flex-col cursor-pointer 
            ${task.index===0 ? 'blueBg whiteFont' : ''}`}>
            ...
            <div className="footer w-100 flex alignCenter justifyBetween">
                <div className="">
                    <div onClick={() => updateTask(task.id, {isCompleted: !task.isCompleted})}
                     className={`fw-500 status cursor-pointer ${task.isCompleted ? 'completed' : 'uncompleted'}`}>
                        {task.isCompleted ? 'completed' : 'uncompleted'}
                    </div>
                </div>
                <div className="icons flex alignCenter justifyCenter">
                    <div className="star cursor-pointer" onClick={() => updateTask(task.id, {isImportant: !task.isImportant})}>
                        {task.isImportant ? 
                        <div>
                    ...
                    <div onClick={editTask} className="threeDot cursor-pointer">
                        <div>
